...
function loadProblem(p) {
    ...
    if (p === 5) {
        setupMapColoring();
        document.getElementById("subtitle").innerText = "CSP Solver";
    }

    if (p === 8) {
        setupNavigation();
        document.getElementById("subtitle").innerText = "BFS / DFS";
    }
...
function setupNavigation() {
    document.getElementById("title").innerText = "SMART NAVIGATION";

    document.getElementById("controls").innerHTML = `
        <div class="section">ADD NODE</div>
        <input id="node">
        <button onclick="addNode()">+ Add Node</button>

        <div class="section">ADD EDGE</div>
        <input id="a" placeholder="From">
        <input id="b" placeholder="To">
        <button onclick="addEdge()">+ Add Edge</button>

        <div class="section">START / GOAL</div>
        <input id="start">
        <input id="goal">

        <button style="background:#059669" onclick="runBFS()">▶ BFS</button>
        <button style="background:#2563eb" onclick="runDFS()">▶ DFS</button>
        <button style="background:#374151" onclick="clearAll()">✕ Clear</button>
    `;
}

function setupMapColoring() {
    document.getElementById("title").innerText = "MAP COLORING";
...
function runBFS() {
    ...
    let res = bfs(s, g);
    showOutput("BFS Path: " + (res.path ? res.path.join(" → ") : "None") + "\nNodes: " + res.explored.join(", "));
}
...
function runDFS() {
    ...
    let g = document.getElementById("goal").value;
    let res = dfs(s, g);
    showOutput("DFS Path: " + (res.path ? res.path.join(" → ") : "None") + "\nNodes: " + res.explored.join(", "));
}
